...
DTC_SWITCH_DESC = (
    "%s - stock code word (Enabled) vs a zero word (Disabled-style).\n\n"
    "DISABLED writes 0x0000 to this code's slot, making it identical to the many "
    "slots the factory leaves empty. This is NOT a proven fault or report disable. "
    "The firmware does not test this code word for nonzero before selecting or "
    "reporting a fault, so detection, failsafe, persistence and code selection can "
    "all remain active, and whether a scan tool still reports the fault with a zero "
    "word is unverified. Treat it as an experimental code-word replacement, not a "
    "DTC-disable control.\n\n"
    "Even in the best case it would suppress the CODE, not the FAULT - whatever "
    "limp-home, pressure or shift behaviour the TCU applies when that bit sets will "
    "still happen. Useful only after a hardware change that leaves a sensor "
    "permanently faulted; a poor idea for silencing a fault you have not diagnosed.\n\n"
    "This is inferred from the table layout and confirmed against the firmware's "
    "report path (which does not gate on a nonzero word); it is not tested on a "
    "vehicle. Note the code you removed before you remove it.\n\n"
